      ...
      <div className='pb-5'>
        <div className='flex justify-between items-center'>
          ...
          <div className='relative py-2 group'>
            <img src={assets.menu_icon} alt="menu" className='max-h-5 cursor-pointer' onClick={(e) => {
              e.stopPropagation();
          ...
          filteredUsers.map((user, index) => (
            <div onClick={() => { setSelectedUser(user); setUnseenMessages(prev => ({ ...prev, [user._id]: 0 })) }} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm
             ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
              ...
              <img src={user?.profilePic || assets.avatar_icon} alt="Profile" className='w-8.75 aspect-square rounded-full' />
